...
### 5.2 ページ編集（主画面）
```
┌ 黄泉のツガイ ▾  run: gemini ▾  巻: 01 ▾   ◀ 0010 / 0104 ▶   [再抽出] [AI修正] [確定] ┐
├──────────────────────────────┬──────────────────────────────────────────┤
│                              │  #  panel  speaker ▾      text          conf basis │
│   キャプチャ画像               │  1    1    ダンジ         ユル！おいこら…  0.95 tail  │
│   ・吹き出し位置に番号マーカー    │  2    1    ユル           おうダンジ…     0.95 tail  │
│   ・行選択とマーカーが連動       │  3    2    ダンジ         また一人で…     0.95 tail  │
│   ・クリックで新規行の座標指定    │  …                                                 │
│   ・ズーム / パン               │  [＋行追加] [－削除] [↑↓並べ替え]                     │
...
M1 は既存データ（`works/黄泉のツガイ` の 2 run）だけで開発・検証できる。
