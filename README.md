...
「台帳の外見を検証」から手動で実行できるほか、抽出ダイアログの「抽出完了後に台帳の外見を検証する」
（既定オン）で抽出の直後に自動実行されます。
...
- ← / → でページ送り（右→左の読み方向。← が次のページ）、巻の切替、M で番号マーカーの表示切替（カーソル付近のマーカーは自動で隠れます）。⌘ / Ctrl + Z で元に戻す、⌘⌫ / Ctrl+Backspace で行削除

Windows 版の配布と非エンジニア向けの利用手順は `docs/user-guide-windows.md` を参照してください。
- このページの再抽出、エクスポート
